...
const getWeights = () => {
    storedWeights = JSON.parse(localStorage.getItem('weights'));

    if (storedWeights.length) {
        for (let i = 0; i < storedWeights.length; i++) {
            const element = storedWeights[i];
            weights.value.push({
